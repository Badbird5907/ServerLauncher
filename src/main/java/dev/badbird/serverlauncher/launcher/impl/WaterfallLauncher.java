...
public class WaterfallLauncher implements Launcher {
    private static final Logger LOGGER = Logger.getLogger("WaterfallLauncher");
    ...
    public void download(LauncherConfig config) {
        ...
        String downloadURL = "https://api.papermc.io/v2/projects/waterfall/versions/%version%/builds/%build%/downloads/waterfall-%version%-%build%.jar"
        ...
        String downloadTarget = config.getDownloadedFileName()
                ...
                .replace("%version%", config.getVersion())
                .replace("%build%", buildNumber + "");
        System.out.println("[WaterfallLauncher] Downloading server jar build #" + buildNumber + " version " + config.getVersion());
        System.out.println("[WaterfallLauncher] Downloading to " + downloadTarget + " from " + downloadURL);
    ...
    public int getLatestBuildNumber(LauncherConfig cfg) throws Exception {
        String urlString = "https://api.papermc.io/v2/projects/waterfall/versions/" + cfg.getVersion();
        ...
        System.out.println("[WaterfallLauncher] Found latest build for PaperMC " + cfg.getVersion() + " #" + largest);
        return largest;
    }
    ...
    public void launch(LauncherConfig config) {
        if (jarName.isEmpty()) {
            System.out.println("[WaterfallLauncher] jarName not set, cannot launch");
            return;
        }
        File jarFile = new File(jarName);
        if (!jarFile.exists()) {
            System.out.println("[WaterfallLauncher] jarFile not found, cannot launch");
            return;
        }
